...
module Logged_1_byte_character_decoder = Logged_character_decoder (struct
  let next () = 1
end)

module Logged_2_byte_character_decoder = Logged_character_decoder (struct
  let next () = 2
end)

module Logged_n_byte_character_decoder = Logged_character_decoder (struct
  let last = ref 0

  let next () = incr last ; !last
end)
...
module Logged_1_byte_character_string_validator =
  E.String_validator (Logged_1_byte_character_decoder)
module Logged_2_byte_character_string_validator =
  E.String_validator (Logged_2_byte_character_decoder)
module Logged_n_byte_character_string_validator =
  E.String_validator (Logged_n_byte_character_decoder)

(** A validator that accepts all strings. *)
module Universal_string_validator =
  E.String_validator (Universal_character_decoder)

(** A validator that rejects all strings. *)
module Failing_string_validator = E.String_validator (Failing_character_decoder)

(** A validator that rejects strings containing the character 'F'. *)
module Selective_string_validator =
  E.String_validator (Selective_character_decoder)
...
module String_validator = struct
  ...
  let test_validate_with_1_byte_characters () =
    ...
    Alcotest.(check (list int))
      "indices"
      !Logged_1_byte_character_decoder.indices
      [9; 8; 7; 6; 5; 4; 3; 2; 1; 0]
  ...
  let test_validate_with_2_byte_characters () =
    ...
    Alcotest.(check (list int))
      "indices"
      !Logged_2_byte_character_decoder.indices
      [8; 6; 4; 2; 0]
  ...
  let test_validate_with_incomplete_string () =
    ...
    Alcotest.check_raises "Validation fails correctly for an incomplete string"
      E.String_incomplete (fun () ->
        Logged_2_byte_character_string_validator.validate "0")
...
module UTF8_UCS_validator = struct
  ...
  let tests = [("test_vaidate", `Quick, test_validate)]
end
  ...
  let test_validate () =
    ...
    while !value <= UCS.max_value +++ 1l do
      if UCS.is_out_of_range !value then
        ...
        Alcotest.check_raises "should fail" E.UCS_value_prohibited_in_UTF8
          (fun () -> validate !value)
      else if XML.is_forbidden_control_character !value then
...
module UTF8_codec = struct
  ...
  let test_encode_decode_cycle_for_value value =
    ...
    if value <> decoded_value then
      Alcotest.fail
        (Printf.sprintf "expected value %06lx but decoded value %06lx\n" value
           decoded_value) ;
    if width <> decoded_width then
      Alcotest.fail
        (Printf.sprintf "expected width %i but decoded width %i\n" width
           decoded_width)
