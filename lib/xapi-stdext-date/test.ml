...
let iso8601_tests =
  let test_of_float_invertible () =
    ...
    check_true "of_float inverts to_float"
    @@ eq (time |> of_float) (time |> of_float |> to_float |> of_float)
  in
  let test_only_utc () =
  ...
  let test_xsi894 () =
    ...
    check_float "to_float assumes UTC" 1607620760.
      (missing_tz_no_dash |> of_string |> to_float) ;
