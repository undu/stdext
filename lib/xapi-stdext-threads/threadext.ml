...
module Alarm = struct
  ...
  let create () = {token= Mutex.create (); queue= []; notifier= None}
              ...
              let nqueue =
                List.filter
                  (fun (clock, callback) ->
                    ...
                    clock > now
                    ||
                    let _ = Thread.create callback () in
                    false)
                  alarm.queue
              in
              alarm.queue <- nqueue ;
...
module Thread = struct
  type t = Running of Thread.t | Pending of pthread
  ...
  let rec run_pendings () =
    if not (PQueue.is_empty !pqueue) then (
      ...
      let to_run =
        ...
        | AlwaysRun ->
            true
        | MaxCapacity (max_threads, _) ->
            c <= now || !running < max_threads
        | WaitCondition f ->
            f () = Now
      in
      if to_run then (
        run_thread t ; run_pendings ()
      ) else
        (* extra logic to avoid starvation or wrongly programmed deadlock *)
        let timeouts, exist, indefs = PQueue.split !pivot !pqueue in
        if
          (not exist)
          || PQueue.cardinal timeouts >= !pre_pivot
             && (run_thread !pivot ; true)
        then
          pivot :=
            if PQueue.is_empty indefs then
              fake_pivot
            else
              PQueue.min_elt indefs ;
        pre_pivot := PQueue.cardinal timeouts
    )

  let exit () =
    Mutex.execute scheduler_token (fun () -> decr running ; run_pendings ()) ;
    Thread.exit ()

  let set_policy p =
    Mutex.execute scheduler_token (fun () ->
        policy := p ;
        run_pendings ())

  let create ?(schedule = Indefinite) f x =
    let finally = Xapi_stdext_pervasives.Pervasiveext.finally in
    let f' x = finally (fun () -> f x) exit in
    Mutex.execute scheduler_token (fun () ->
        run_pendings () ;
        let timeout =
          match schedule with
        ...
        let timeout =
          if timeout = 0. then
            ...
            | MaxCapacity (max_threads, max_wait_opt) -> (
                if !running < max_threads && PQueue.is_empty !pqueue then
                  0.
                else
                  match max_wait_opt with
                  | None ->
                      timeout
                  | Some t ->
                      min timeout t
        ...
        if timeout <= 0. then (
          ...
          incr running ; Running t
        ) else
          let deadline =
            if timeout < max_float then
              timeout +. Unix.time ()
            else
              max_float
          in
...
functor
  ...
  struct
      ...
      let _ =
        Thread.create
          (fun () ->
            ...
            Thread.join tid ;
            List.iter
              (fun (_, t, _) -> if tid = t then Hashtbl.remove ref_table xref)
    ...
    let stop xref =
      try
        let mut, _, exit_ref = Hashtbl.find ref_table xref in
        Mutex.execute mut (fun () -> exit_ref := true)
      with Not_found -> ()

    (** Replace a thread with another one *)
    let update xref fn = stop xref ; start xref fn
  end
